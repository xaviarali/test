            ...
            if( L1[code[g]] != null)
	        {
                for(var i = 0,len = L1[code[g]].length; i < len; i++)
                {
			               ...
			               {	 
			                 for(var b = 0,xxlen = L3[code[g]+(i+1)+''+(k+1)].length; b < xxlen; b++)
			                {
								 ...
								 if( (b+1) > 9 ) level4 = telnetCorrespondence[(b+1)%10];                                
							      bindEventListeners('#'+code[g]+(i+1)+''+(k+1)+''+(b+1),level1+','+level2+','+sessionStorage.getItem("tabId")+','+code[g]+','+level3+','+level4);
			                }
                           }
			               else
			              {
							...
							if( (i+1) > 9 ) level2 = telnetCorrespondence[(i+1)%10];
							if( (k+1) > 9 ) level3 = telnetCorrespondence[(k+1)%10];
							bindEventListeners('#'+code[g]+(i+1)+''+(k+1),level1+','+level2+','+sessionStorage.getItem("tabId")+','+code[g]+','+level3);
			              }
			          }
                    }
			        else
			        {
				            ...
				            if( (g+1) > 9 ) level1 = telnetCorrespondence[(g+1)%10];
							if( (i+1) > 9 ) level2 = telnetCorrespondence[(i+1)%10];
							bindEventListeners('#'+code[g]+(i+1),level1+','+level2+','+sessionStorage.getItem("tabId")+','+code[g]);
			        }
	             }
	        }
        }
...
function promptBeforeLeaving()
{
	window.onbeforeunload = function(){
		if(true)
		{
			...
			console.log("Close the Telnet98 Session");
		}
		else
		{
